    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center space-y-3 max-w-2xl mx-auto mb-10 sm:mb-14">
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full border border-[#E5E7EB] bg-slate-50 text-[10px] sm:text-[11px] font-mono font-bold text-slate-500 tracking-wider uppercase select-none">
          ...
          <p className="text-slate-500 text-xs sm:text-sm leading-relaxed font-normal">
            ScreenToSkill automatically adapts challenge difficulties—ranging
            from sensory auditory play for pre-school toddlers to high-level
            science, grammar, and algebraic reasoning for teens.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 items-stretch">
          {earlyGroups.map((group) => (
            <div
              key={group.index}
              className="group relative bg-white border border-[#E5E7EB] hover:border-slate-200/60 rounded-xl hover:shadow-lg transition-all duration-300 flex flex-col overflow-hidden"
            >
              {/* Image segment - elevated image height, edge-to-edge, clean aspect ratio */}
              <div className="relative w-full h-[200px] sm:h-[260px] lg:h-[350px] overflow-hidden bg-slate-50 rounded-t-xl border-b border-[#E5E7EB]">
                <Image
                  src={group.imageUrl}
              ...
              <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between text-left">
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <span className="block text-[9px] font-mono font-bold text-[#22C55E] tracking-widest uppercase">
                      {group.groupName}
                    </span>
                  ...
                  <div className="bg-slate-50 group-hover:bg-slate-50/50 border border-[#E5E7EB] rounded-lg p-3.5">
                ...
                <div className="border-t border-[#E5E7EB] pt-3.5 mt-4">
                  <div className="flex flex-wrap gap-x-4 gap-y-1.5">
                    {group.specs.map((spec, si) => (
                      <div
                        key={si}
                        className="flex items-center space-x-1.5 text-xs font-normal text-slate-550"
                      >
              ...
              <div className="absolute bottom-3 right-6 pointer-events-none select-none">
                <span className="text-[9px] font-mono text-slate-300 font-bold tracking-widest">
                  {group.code}
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-stretch">
          {matureGroups.map((group) => (
            ...
            >
              ...
              <div className="relative w-full h-[180px] sm:h-[230px] lg:h-[320px] overflow-hidden bg-slate-50 rounded-t-xl border-b border-[#E5E7EB]">
                <Image
                  src={group.imageUrl}
                  alt={`${group.title} representation`}
                  fill
